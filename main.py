...
def not_completed_levels():
    SCREEN.fill((0, 255, 255))
    my_font = pygame.font.SysFont('Comic Sans MS', 100)
    font_render = my_font.render('Complete Level 3', False, (0, 0, 0))
...
def free_play_normal(segment):
    ...
    background = pygame.transform.scale(pygame.image.load("Images/Background_images/menu_background.png"),
                                        (1280, 736)).convert_alpha()
    explosion = pygame.transform.scale(pygame.image.load("Images/Background_images/explosion.png"),(4000, 3000)).convert_alpha()
    ...
    shop_rect = shop_text.get_rect(center=(100, 650))

    coins_rect = shop_text.get_rect(center=(230, 650))

    explosion_text = get_font(20).render("explosion distance:", True, "#b68f40")
    explosion_rect = shop_text.get_rect(center=(100, 700))

    distance_rect = shop_text.get_rect(center=(480, 700))
    ...
    free_play_music()
    SCREEN.blit(explosion, pygame.Rect(100 + entities.cam[0], 250 + entities.cam[1], 600, 400))
    previous_time = pygame.time.get_ticks() + 3
    enemy_list = []
    explosions = []
    explosions.append(pygame.Rect(-400000, -800, 1700, 3300))
    counter = 0

    while 1:
        explosion_distance = (round((player.rect[0] - (explosions[0][0] + 1290))/32))
        coins_text = get_font(20).render(str(COINS), True, "#b68f40")
        distance_text = get_font(20).render(str(explosion_distance), True, "#b68f40")
        ...
        for platform in platforms:
            if (platform.rect[0]) - player.rect[0] < 1400:
                drawn_entities.append(platform)
            if player.rect[0] - platform.rect[0] > 1060:
        ...
        for p in drawn_entities:
            ...
            if explosions[0].colliderect(player):
                dead = True
            if player.rect[0] - p.rect[0] > 1050:
                p.kill()
                counter += 1
        ...
        if explosion_distance < 40:
            explosion_sound()
            SCREEN.blit(explosion, pygame.Rect(explosions[0][0] + entities.cam[0], explosions[0][1] + entities.cam[1], 32, 736))
...
class PlatEnemy(Entity):
    def __init__(self, pos, *groups):
        ...
        self.image = pygame.transform.scale(pygame.image.load("Images/Background_images/enemy_turret.png"), (32, 32)).convert_alpha()
